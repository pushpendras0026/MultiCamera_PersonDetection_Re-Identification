...
import pandas as pd
import collections
import collections.abc          # ← FIX: Python 3.10+ moved Mapping here
import random
import math
## For torch lib
import torch
...
class Video_train_Dataset(Dataset):
    def __init__(self, db_txt, info, transform, seq_len=6, track_per_class=4,
                 flip_p=0.5, delete_one_cam=False, cam_type='normal'):
        ...
        for i in range(len(info)):
            ...
            if F < seq_len:
                ...
                for s in range(seq_len):
                    ...
                    sample_clip.append(list(pool))
            self.info.append(np.array([np.array(sample_clip, dtype=np.int64), info[i][2], info[i][3]], dtype=object))

        self.info = np.array(self.info, dtype=object)
        self.transform = transform
        self.n_id = id_count
        self.n_cam = len(np.unique([x[2] for x in self.info]))
        self.flip_p = flip_p
    ...
    def __getitem__(self, ID):
        sub_info = self.info[np.array([x[1] for x in self.info]) == ID]

        if self.cam_type == 'normal':
            tracks_pool = list(np.random.choice(sub_info[:, 0], self.track_per_class))
        elif self.cam_type == 'two_cam':
            unique_cam = np.random.permutation(np.unique([x[2] for x in sub_info]))[:2]
            tracks_pool = (
                list(np.random.choice(sub_info[np.array([x[2] for x in sub_info]) == unique_cam[0], 0], 1)) +
                list(np.random.choice(sub_info[np.array([x[2] for x in sub_info]) == unique_cam[1], 0], 1))
            )
        elif self.cam_type == 'cross_cam':
            unique_cam = np.random.permutation(np.unique([x[2] for x in sub_info]))
            ...
            for i in range(self.track_per_class):
                cam_mask = np.array([x[2] for x in sub_info]) == unique_cam[i]
                tracks_pool += list(np.random.choice(sub_info[cam_mask, 0], 1))
        ...
        for track_pool in tracks_pool:
            ...
            imgs = [self.transform(Image.open(path).convert('RGB')) for path in self.imgs[number]]
            imgs = torch.stack(imgs, dim=0)
            if random.random() < self.flip_p:
                imgs = torch.flip(imgs, dims=[3])
            one_id_tracks.append(imgs)

        return (torch.stack(one_id_tracks, dim=0),
                ID * torch.ones(self.track_per_class, dtype=torch.int64))
...
def Video_train_collate_fn(data):
    # ← FIX: collections.abc.Mapping instead of collections.Mapping
    if isinstance(data[0], collections.abc.Mapping):
        t_data = [tuple(d.values()) for d in data]
        values = Video_train_collate_fn(t_data)
...
def Get_Video_train_DataLoader(db_txt, info, transform, shuffle=True, num_workers=4,
                               seq_len=6, track_per_class=4, class_per_batch=8):
    dataset = Video_train_Dataset(db_txt, info, transform, seq_len, track_per_class)
    use_persistent = num_workers > 0
    dataloader = DataLoader(
        dataset, batch_size=class_per_batch, collate_fn=Video_train_collate_fn,
...
# ──────────────────────────────────────────────────────────────────────────────
# Test Dataset – Random Repeat Sampling (rrs) – fast in-training validation
# ──────────────────────────────────────────────────────────────────────────────
...
class Video_test_rrs_Dataset(Dataset):
    def __init__(self, db_txt, info, query, transform, seq_len=6, distractor=True):
        ...
        self.info = []
        for i in range(len(info)):
            if not distractor and info[i][2] == 0:
                continue
            sample_clip = []
            F = info[i][1] - info[i][0] + 1
            if F < seq_len:
        ...
        self.info = np.array(self.info, dtype=object)
        self.transform = transform
        self.n_id = len(np.unique([x[1] for x in self.info]))
        self.n_tracklets = len(self.info)
        ...
        if not distractor:
            ...
            for i in self.query_idx:
                if i < zero[0]:
                    ...
                    new_query.append(i - len(zero))
            self.query_idx = np.array(new_query)

    def __getitem__(self, idx):
        clips = self.info[idx][0]
        imgs = [self.transform(Image.open(path).convert('RGB')) for path in self.imgs[clips[:, 0]]]
        imgs = torch.stack(imgs, dim=0)
        label = int(self.info[idx][1]) * torch.ones(1, dtype=torch.int32)
        cam = int(self.info[idx][2]) * torch.ones(1, dtype=torch.int32)
        paths = np.stack([self.imgs[clips[:, 0]]], axis=0)
...
def Video_test_rrs_collate_fn(data):
    if isinstance(data[0], collections.abc.Mapping):
        t_data = [tuple(d.values()) for d in data]
        values = Video_test_rrs_collate_fn(t_data)
        return {key: value for key, value in zip(data[0].keys(), values)}
    else:
...
def Get_Video_test_rrs_DataLoader(db_txt, info, query, transform, batch_size=64, shuffle=False,
    ...
    use_persistent = num_workers > 0
    dataloader = DataLoader(
        dataset, batch_size=batch_size, collate_fn=Video_test_rrs_collate_fn,
        shuffle=shuffle, worker_init_fn=seed_worker, num_workers=num_workers,
        pin_memory=True, persistent_workers=use_persistent)
    return dataloader


# ──────────────────────────────────────────────────────────────────────────────
# Test Dataset – All frames (full evaluation)
# ──────────────────────────────────────────────────────────────────────────────
...
def Video_test_all_collate_fn(data):
    if isinstance(data[0], collections.abc.Mapping):
        t_data = [tuple(d.values()) for d in data]
        values = Video_test_all_collate_fn(t_data)
        return {key: value for key, value in zip(data[0].keys(), values)}
    else:
        imgs, label, cam, paths = zip(*data)
        imgs = torch.cat(imgs, dim=0)
...
def Get_Video_test_all_DataLoader(db_txt, info, query, transform, batch_size=1, shuffle=False,
                                   num_workers=4, seq_len=6, distractor=True):
    dataset = Video_test_all_Dataset(db_txt, info, query, transform, seq_len, distractor=distractor)
    use_persistent = num_workers > 0
    dataloader = DataLoader(
        dataset, batch_size=batch_size, collate_fn=Video_test_all_collate_fn,
        shuffle=shuffle, worker_init_fn=seed_worker, num_workers=num_workers,
        pin_memory=True, persistent_workers=use_persistent)
    return dataloader
...
class RandomErasing(object):
    """Random Erasing Data Augmentation (Zhong et al., arXiv:1708.04896)."""
